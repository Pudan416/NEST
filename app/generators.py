    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                return alternatives[0]["message"].get("text", "Error: No text found.") if alternatives else "Error: Invalid response structure."
...
async def here_maps_nearby_places(latitude: float, longitude: float, category: str, radius: int = 1000):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                items = response.json().get("items", [])
                return [item for item in items if geodesic((latitude, longitude), (item["position"]["lat"], item["position"]["lng"])).meters <= radius]
...
async def yandex_gpt_location_info(city: str, street: str, poi_name: str, poi_address: str):
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "text": f"Street: {street}, City: {city}, POI: {poi_name}, Address: {poi_address}"},
        ],
    }
